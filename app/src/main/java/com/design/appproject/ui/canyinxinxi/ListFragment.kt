...
import com.design.appproject.ui.canyinxinxi.ListFilterDialog
...
import com.design.appproject.databinding.CanyinxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.canyinxinxi.ListModel
import com.design.appproject.ui.canyinxinxi.ListAdapter
 ...
 * 餐饮信息列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_CANYINXINXI)
class ListFragment : BaseBindingFragment<CanyinxinxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("餐饮信息",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("canyinxinxi","新增"))|| Utils.isAuthFront("canyinxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_CANYINXINXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_CANYINXINXI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("canyinxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_CANYINXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun CanyinxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("canyinxinxi", requestParams)
        }.otherwise {
                mListModel.list("canyinxinxi", requestParams)
        }
    }
}