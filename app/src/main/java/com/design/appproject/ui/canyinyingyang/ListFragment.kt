...
import com.design.appproject.ui.canyinyingyang.ListFilterDialog
...
import com.design.appproject.databinding.CanyinyingyangcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.canyinyingyang.ListModel
import com.design.appproject.ui.canyinyingyang.ListAdapter
 ...
 * 餐饮营养列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_CANYINYINGYANG)
class ListFragment : BaseBindingFragment<CanyinyingyangcommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("菜品名称","caipinmingcheng")/*查询索引*/
    ...
    override fun initEvent() {
        setBarTitle("餐饮营养",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("canyinyingyang","新增"))|| Utils.isAuthFront("canyinyingyang","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_CANYINYINGYANG).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_CANYINYINGYANG)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("canyinyingyang",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_CANYINYINGYANG,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun CanyinyingyangcommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("caipinmingcheng","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("caipinmingcheng")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("canyinyingyang", requestParams)
        }.otherwise {
                mListModel.list("canyinyingyang", requestParams)
        }
    }
}