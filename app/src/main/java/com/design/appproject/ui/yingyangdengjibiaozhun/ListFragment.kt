...
import com.design.appproject.ui.yingyangdengjibiaozhun.ListFilterDialog
...
import com.design.appproject.databinding.YingyangdengjibiaozhuncommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.yingyangdengjibiaozhun.ListModel
import com.design.appproject.ui.yingyangdengjibiaozhun.ListAdapter
 ...
 * 营养等级标准列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_YINGYANGDENGJIBIAOZHUN)
class ListFragment : BaseBindingFragment<YingyangdengjibiaozhuncommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("等级名称","dengjimingcheng")/*查询索引*/

    override fun initEvent() {
        setBarTitle("营养等级标准",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("yingyangdengjibiaozhun","新增"))|| Utils.isAuthFront("yingyangdengjibiaozhun","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YINGYANGDENGJIBIAOZHUN).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YINGYANGDENGJIBIAOZHUN)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("yingyangdengjibiaozhun",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_YINGYANGDENGJIBIAOZHUN,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun YingyangdengjibiaozhuncommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("dengjimingcheng","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("dengjimingcheng")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("yingyangdengjibiaozhun", requestParams)
        }.otherwise {
                mListModel.list("yingyangdengjibiaozhun", requestParams)
        }
    }
}